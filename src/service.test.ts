...
import { Data, Item, PaginatedItems, Service } from "./service.ts";
...
await Deno.test("find", async (t) => {
  ...
  for (const tc of arr) {
    await t.step(`${tc.name} ${JSON.stringify(tc.params)}`, () => {
      if (tc.data) {
        db.data = tc.data;
      } else {
        reset();
...
await Deno.test("updateById", async () => {
  reset();
...
await Deno.test("patchById", async () => {
  reset();
  const post = { id: "xxx", title: "updated post" };
