...
import { getAvailablePort as getPort } from "get-port";
...
Deno.test("createApp", async (t) => {
  ...
  const { containerForManualResolving, promise } = (() => {
    const containerForManualResolving = [] as (() => void)[];
    const promise = new Promise<void>((resolve, reject) => {
      try {
        const server = app.listen(port);
        containerForManualResolving.push(() => {
          server.close(() => {
            resolve();
          });
        });
  ...
  for (const tc of arr) {
    await t.step(`${tc.method} ${tc.url}`, async () => {
